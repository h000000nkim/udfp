    ...
    with zf:
        ...
        for part in _REQUIRED_PARTS:
            if part not in names:
                violations.append(
                    RuleViolation("D1", "", f"필수 파트 누락: {part}")
                )
    return violations
...
def check_d3(path: str) -> list[RuleViolation]:
    ...
    with zf:
        ...
        for rels_path in rels_files:
            ...
            rels_dir = rels_path.rsplit("/", 1)[0].replace("_rels", "") if "/" in rels_path else ""
...
class DocxValidationReport:
    """Aggregated result of D1-D3 validation checks."""
...
def validate_docx(path: str) -> DocxValidationReport:
    """Run all DOCX D-rules (D1-D3) and return a consolidated report.
