...
_PREAMBLE_CTRL_IDS = frozenset({
    0x73656364,  # secd (LE u32)
    0x636F6C64,  # cold (LE u32)
})


def fix_preamble_text_i6(stream: bytes) -> bytes:
    """Fix I6: strip regular text from preamble paragraph.

    The first L0 paragraph containing 'secd' should have only inline
    control characters + PARA_END. If regular text is mixed in, this
    fixer strips text and non-essential controls (pgnp, nwno, etc.)
    from preamble PARA_TEXT, keeping only secd/cold/head markers +
    PARA_END. Paragraph count is preserved (함정 10 준수).

    Non-essential CTRL_HEADER records (those not matching preamble
    controls) are removed. Their associated text is lost, but this is
    acceptable for repair of structurally corrupt originals.
    """
    records = list(iter_records(stream))
    if not records:
        return stream
    ...
    first_para_end: int = 0

    for i, rec in enumerate(records):
        if rec.tag_id != HWPTAG_PARA_HEADER or rec.level != 0:
            continue
        has_secd = False
        local_pt: int | None = None
        ph_level = rec.level
        end = i
        for j in range(i + 1, len(records)):
            if records[j].level <= ph_level and j > i:
                end = j
                break
            if records[j].tag_id == HWPTAG_PARA_TEXT and records[j].level == ph_level + 1:
                local_pt = j
            if records[j].tag_id == HWPTAG_CTRL_HEADER and records[j].level == ph_level + 1:
                if len(records[j].payload) >= 4:
                    cid = struct.unpack_from("<I", records[j].payload, 0)[0]
                    if cid == 0x73656364:
                        has_secd = True
        else:
            end = len(records)
    ...
    if preamble_idx is None or pt_idx is None:
        return stream

    pt_payload = records[pt_idx].payload
    pos = 0
    has_regular = False
    while pos + 2 <= len(pt_payload):
        ch = struct.unpack_from("<H", pt_payload, pos)[0]
        if ch in _INLINE_OBJ_CODES:
            pos += 16
        elif ch in (0x0000, 0x0009, 0x000A, 0x000D, 0x001F):
            pos += 2
        else:
            has_regular = True
            break
    ...
    ph_level = records[preamble_idx].level

    kept_ctrl_ids: set[int] = set()
    for j in range(preamble_idx + 1, first_para_end):
        rec = records[j]
        if rec.tag_id == HWPTAG_CTRL_HEADER and rec.level == ph_level + 1 and len(rec.payload) >= 4:
            cid = struct.unpack_from("<I", rec.payload, 0)[0]
            kept_ctrl_ids.add(cid)

    preamble_bytes = bytearray()
    pos = 0
    while pos + 2 <= len(pt_payload):
        ch = struct.unpack_from("<H", pt_payload, pos)[0]
        if ch in _INLINE_OBJ_CODES:
            if pos + 16 <= len(pt_payload):
                cid = struct.unpack_from("<I", pt_payload, pos + 2)[0]
                if cid in kept_ctrl_ids:
                    preamble_bytes.extend(pt_payload[pos : pos + 16])
            pos += 16
        elif ch == 0x000D:
            pos += 2
        elif ch in (0x0000, 0x0009, 0x000A, 0x001F):
            pos += 2
        else:
            pos += 2

    preamble_bytes.extend(b"\x0d\x00")

    records[pt_idx] = HwpRecord(
        HWPTAG_PARA_TEXT, records[pt_idx].level, bytes(preamble_bytes), 0
    )

    preamble_char_count = len(preamble_bytes) // 2
    old_ph = bytearray(records[preamble_idx].payload)
    old_dw = struct.unpack_from("<I", old_ph, 0)[0]
    msb = old_dw & 0x80000000
    struct.pack_into("<I", old_ph, 0, msb | (preamble_char_count & 0x3FFFFFFF))
    struct.pack_into("<H", old_ph, 12, 1)
    records[preamble_idx] = HwpRecord(
        HWPTAG_PARA_HEADER, ph_level, bytes(old_ph), 0
    )

    for j in range(preamble_idx + 1, first_para_end):
        if records[j].tag_id == HWPTAG_PARA_CHAR_SHAPE and records[j].level == ph_level + 1:
            first_csid = struct.unpack_from("<I", records[j].payload, 4)[0] if len(records[j].payload) >= 8 else 0
            records[j] = HwpRecord(
                HWPTAG_PARA_CHAR_SHAPE, records[j].level,
                struct.pack("<II", 0, first_csid), 0,
            )
            break
