...
from sandbox.migration import v1_to_v2, v2_to_v1
from sandbox.migration._blocks import ExtensionCollector, ExtensionLookup
from sandbox.migration._formats import (
    block_format_to_v1,
    block_format_to_v2,
    cell_format_to_v1,
    cell_format_to_v2,
...
from sandbox.migration._inlines import inline_to_v1, inline_to_v2
from sandbox.migration._metadata import metadata_to_v1, metadata_to_v2
from sandbox.migration._types import (
    color_to_str,
    format_mm,
    format_pct,
    format_pt,
...
from sandbox.migration._verbatim import verbatim_to_v1, verbatim_to_v2
from sandbox.schema.types import Color, Ratio
...
class TestPositionConversion:
    def test_v2_strips_extensions(self):
        ...
        exts = collector.to_extensions()
        assert "hwp" in exts
        hwp_ext = exts["hwp"]
        assert "b_0001" in hwp_ext.positions
        assert hwp_ext.positions["b_0001"].width_relto == "page"

    def test_v1_restores_extensions(self):
        from sandbox.schema import extensions as ext

        hwp = ext.HwpExtension(
            positions={
                "b_0001": ext.HwpExtension.PositionExt(
                    width_relto="page",
                    height_relto="paper",
                    restrict_in_page=True,
                ),
        ...
        lookup = ExtensionLookup({"hwp": hwp})
...
class TestInlineConversion:
    ...
    def test_equation_inline_hwp_script(self):
        v1_inl = v1.EquationInline(latex="x^2", hwp_script="x^2")
        collector = ExtensionCollector("hwp")
        v2_inl = inline_to_v2(v1_inl, collector, "b_0001", 0)
        assert v2_inl.latex == "x^2"

        exts = collector.to_extensions()
        assert exts["hwp"].inline_equations["b_0001:0"].hwp_script == "x^2"

        lookup = ExtensionLookup(exts)
        v1_back = inline_to_v1(v2_inl, lookup, "b_0001", 0)
...
def _roundtrip_block(v1_block, source_format="hwp"):
    ...
    from sandbox.migration._blocks import block_to_v1, block_to_v2
...
class TestBlockRoundtrip:
    ...
    def test_table_with_extensions(self):
        blk = v1.TableBlock(
            type="table", id="t2",
            border_fill_id=5,
            rows=[v1.TableRow(cells=[
                v1.TableCell(id="c1", row_span=1, col_span=1, width=100.0, height=50.0, content=[]),
            ])],
        )
        from sandbox.migration._blocks import block_to_v1, block_to_v2
        ...
        exts = collector.to_extensions()
        assert exts["hwp"].blocks["t2"].border_fill_id == 5
    ...
    def test_equation_block(self):
        blk = v1.EquationBlock(
            type="equation", id="eq1", latex="E=mc^2", hwp_script="E=mc^2",
        )
        from sandbox.migration._blocks import block_to_v1, block_to_v2
        ...
        v2_blk = block_to_v2(blk, collector)
        exts = collector.to_extensions()
        assert exts["hwp"].inline_equations["eq1"].hwp_script == "E=mc^2"
...
class TestDocumentRoundtrip:
    ...
    def test_document_with_extensions(self):
        v1_doc = v1.UdfDocument(
            source_format="hwp",
            blocks=[
        ...
        v2_doc = v1_to_v2(v1_doc)
        assert "hwp" in v2_doc.extensions
        hwp = v2_doc.extensions["hwp"]
        assert hwp.inline_equations["eq1"].hwp_script == "x SUPS 2"
        assert hwp.blocks["t1"].border_fill_id == 3
...
class TestExtensionCollectorLookup:
    ...
    def test_collector_hwpx(self):
        ...
        assert "hwpx" in exts
        from sandbox.schema.extensions import HwpxExtension
