...
OUT = "outputs/미적분1_김도현/김도현_미적분1_최종본_양식맞춤.docx"
...
def set_cell_borders(cell, color="B7B7B7", size="6"):
    tc = cell._tc
...
def format_paragraph(paragraph, size=10.5, bold=False, color="000000", align=None, before=0, after=6, line=1.15):
...
def add_kv_table(doc):
    table = doc.add_table(rows=3, cols=4)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.autofit = False
    set_table_width(table, 9360)
    widths = [1800, 2880, 1800, 2880]
    labels = [("과목", "미적분Ⅰ"), ("성명", "김도현"), ("학년", "2학년"), ("진로", "전기전자학, 전자공학"), ("제출 형식", "한글"), ("작성 유형", "심층 탐구 보고서")]
    idx = 0
    for row in table.rows:
        for ci, cell in enumerate(row.cells):
            cell.width = Cm(widths[ci] / 567)
            cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
            set_cell_borders(cell)
            set_cell_margins(cell)
            if ci % 2 == 0:
                set_cell_shading(cell, "EAF2F8")
            text = labels[idx // 2][idx % 2] if idx < len(labels) * 2 else ""
            cell.text = text
            for p in cell.paragraphs:
                format_paragraph(p, size=9.5, bold=(ci % 2 == 0), align=WD_ALIGN_PARAGRAPH.CENTER if ci % 2 == 0 else WD_ALIGN_PARAGRAPH.LEFT, after=0)
            idx += 1
    return table


def add_section_table(doc, title, paragraphs):
    ...
    for cell in (header, body):
        ...
        cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
    set_cell_shading(header, "D9EAF7")
    header.text = title
    for p in header.paragraphs:
        format_paragraph(p, size=11, bold=True, color="17365D", after=0)

    body.text = ""
    if title == "참고자료":
        for idx, para in enumerate(paragraphs, 1):
            p = body.add_paragraph(f"{idx}. {para}")
            format_paragraph(p, size=9.5, after=4, line=1.15)
    else:
        for para in paragraphs:
            p = body.add_paragraph(para)
            if para.startswith("V("):
                format_paragraph(p, size=10.5, bold=True, color="1F4E79", align=WD_ALIGN_PARAGRAPH.CENTER, before=2, after=6)
            else:
                format_paragraph(p, size=10.2, after=6, line=1.18)
    if body.paragraphs and not body.paragraphs[0].text:
        p = body.paragraphs[0]
...
def build():
    ...
    title_run = title.add_run("미적분Ⅰ의 원리와 응용에 관한 심층 탐구 보고서")
    ...
    for title, paragraphs in CONTENT.items():
        add_section_table(doc, title, paragraphs)
