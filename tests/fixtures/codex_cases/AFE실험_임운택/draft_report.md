...
### 교과 개념 적용 과정에서 생긴 문제의식

화학에서 배운 침전 반응만으로 보면 탄산칼슘 형성은 칼슘 이온과 탄산 이온의 만남으로 설명할 수 있다. 그러나 실제 실험에서는 이온 농도뿐 아니라 pH, 빛의 세기, 온도, 생물의 활력, 젤라틴 비드의 구조가 함께 영향을 준다. 생명과학에서 다루는 광합성은 산소 생성 반응에 머물지 않고 주변의 탄소 순환과 무기물 형성에도 관여할 수 있다.

이 점에서 이번 탐구는 화학적 침전 반응과 생명체의 대사 작용을 함께 다루는 활동이다. 건축공학적으로도 같은 원리가 중요하다. 실제 재료는 한 가지 반응식으로만 성능이 결정되지 않는다. 습도, 온도, 공극, 균열 폭, 재료 내부의 이동 경로 등 여러 조건이 겹쳐 내구성과 강도가 달라진다. 따라서 실험 과정에서 독립변인, 종속변인, 통제변인을 분명히 구분하고, 결과 해석 범위를 신중하게 정하는 것이 필요하다고 판단하였다.
...
다만 실제 측정값이 없는 상태에서는 결과를 확정적으로 말하기 어렵다. 후속 실험에서는 pH 값을 시간대별로 기록하고, 관입 저항을 수치화하며, 현미경 사진을 같은 배율로 비교해야 한다. 또한 젤라틴 비드에서 확인한 침전이 실제 시멘트계 재료나 다공성 콘크리트에서도 비슷하게 작용하는지 검토해야 한다.
...
- 실제 pH 값, 관입 저항 수치, 현미경 사진이 제공되면 VI장 결과 표를 정량 결과 중심으로 보완해야 한다.
- 최종 HWP 양식에 넣을 때는 첨부 사진 2장을 IV장 실험 설계 부분에 배치하고, 실제 실험 사진이 있다면 VI장 결과 부분에 추가하는 것이 좋다.
